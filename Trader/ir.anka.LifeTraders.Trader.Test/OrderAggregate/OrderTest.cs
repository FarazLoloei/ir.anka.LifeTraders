...
public class OrderTest : OrderTestProvider
{
    ...
    public void TestOrderConstructor(Guid accountId,
                                     string login,
                                     long ticket,
                                     string symbol,
                                     double openPrice,
    ...
    {
        var order = new Order(accountId, login, ticket, symbol, openPrice, openDateTime, closePrice, closeDateTime, closeVolume, stopLoss, takeProfit, stopLimitPrice,
        ...
        AssertOrderConstructorTest(accountId, login, ticket, symbol, openPrice, openDateTime, closePrice, closeDateTime, closeVolume, stopLoss, takeProfit, stopLimitPrice,
            placedType, orderType, dealType, orderState, lots, contractSize, expertId, digits, expirationType, fillPolicy, volume, profit,
            profitRate, swap, commission, closeComment, comment, requestId, expirationDateTime, orderDealIn, orderDealOut, order);
    }

    private void AssertOrderConstructorTest(Guid accountId,
                                            string login,
                                            long ticket,
                                            string symbol,
                                            double openPrice,
                                            ...
                                            Order expectedOrder)
    {
        ...
        expectedOrder.AccountId.Should().NotBeEmpty().And.Be(accountId);
    ...
    [Theory]
    [ClassData(typeof(EmptyNullOrderTestCases))]
    public void TestOrderValidator(Guid accountId,
                                   string login,
                                   long ticket,
                                   string symbol,
                                   double openPrice,
                                   ...
                                   OrderDeal orderDealOut)
    {
        Action act = () => new Order(accountId, login, ticket, symbol, openPrice, openDateTime, closePrice, closeDateTime, closeVolume, stopLoss,
                                     takeProfit, stopLimitPrice, placedType, orderType, dealType, orderState, lots, contractSize, expertId, digits,
                                     expirationType, fillPolicy, volume, profit, profitRate, swap, commission, closeComment, comment, requestId,
                                     expirationDateTime, orderDealIn, orderDealOut, orderValidator.Object, sharedValidator.Object);
